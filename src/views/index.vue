    ...
    <script>
        ...
        */
                    ...
                    for (let l in autonmovepartofcycle) {
                        for (let m in movepartofcycle) {
                            for (let n in grabamount) {
                                ...
                                //teleop
                                scenario.time = 135;
                                scenarioaction[encodescenario(""+i+j+k+l+m+n)] = (scenarioaction[encodescenario(""+i+j+k+l+m+n)] ?? "") + " teleop:";
        ...
        function scorepiece(ifauton0ifteleop1,i, j, k,l, m,n, holdingpiecesatbeginningofteleop) {
            ...
            scenarioaction[encodescenario(""+i+j+k+l+m+n)] = (scenarioaction[encodescenario(""+i+j+k+l+m+n)] ?? "") + " scoredpiece;"
        }
        function balance(ifauton0ifteleop1, i,j,k,l,m,n) {
            ...
            scenariospoints.push({ x: 150 - scenario.time, y: scenario.points });
            scenarioaction[encodescenario(""+i+j+k+l+m+n)] = (scenarioaction[encodescenario(""+i+j+k+l+m+n)] ?? "") + " balance;"
        
        }
        //try and tell the length of the stuff
    </script>
  </div>
</template>
